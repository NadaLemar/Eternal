...
- **완성**: 홈페이지, 전체 메뉴 구조(좌측 레일 네비게이션), 무기·코스튬·퀘스트·업적
  4개 섹션의 검색/필터 기능, **강화 시뮬레이터**(플러스업 확률 기반 기대비용 계산기
  + 실전 랜덤 시뮬레이터), **제작 재료**(리마스터 반지 4종 재료표), **거래 게시판**
  (El 전용 판매/구매 게시판, Firebase 연동 — 설정 필요), **시세**(거래 게시판 데이터를
  자동 집계한 아이템별 시세표, 거래 게시판과 같은 Firebase 설정 공유)
- **실제 데이터 반영됨**:
  - `weapons.json` — mgame 공식 게임DB 기반 CL불법무기 31종 (8~12등급, 2차 공격
    모드가 있는 복합 무기 포함)
  - `costumes.json` — 날개의상 16종 × 공격형/치명형/체력형 = 48건
...
- **준비중 페이지**: 방어구, 악세서리, 캐릭터·용병, 지역·어썰트, 계산기는 자리표시
  페이지만 만들어 두었습니다. 데이터가 준비되면 `weapons.html`과 같은 방식으로
  채울 수 있습니다.
